...
def main():
    os.chdir('..')
    current = os.getcwd()
    path = os.path.join(current,"Image_Data/*/")
    #path = "/data/szaman5/training_data/"
    directories = get_directories(path)

    output_dir = os.path.join(current,"Image_Data/Training_Data/")
    #print(output_dir)
   # print(directories[2:-1])
    ...
    while(count < len(directories)):
        temp = 0
        processes = []
        while(count + temp < len(directories) and temp < 16):
            station_name = directories[count+temp].split("/")[-2]
            directory_paths = directories[count+temp]
            if (station_name != '*' and station_name[-2] == '0'):
                lst_path = directories[count+temp]+station_name+".lst"
                cla_path = directories[count+temp]+station_name+".cla"
                tempP = Process(target=extractor,args=(cla_path,lst_path, output_dir, directory_paths, station_name))
                processes.append(tempP)
                tempP.start()
                temp +=1
        # print("batch")
        for k in processes:
